...
class MethodChannelFlutterVpn extends FlutterVpnPlatform {
  ...
  Future<bool> setKillSwitch(bool enabled) async {
    try {
      ...
      debugPrint('Error setting kill switch: $e');
      return false;
    }
  }
  ...
  @override
  Future<bool> get killSwitchEnabled async {
    try {
      final result =
          await methodChannel.invokeMethod<bool>('getKillSwitchState');
      return result ?? false;
    } catch (e) {
  ...
  /// Get current VPN status including kill switch state
  Future<Map<String, dynamic>> getVpnStatus() async {
    try {
      final state = await currentState;
      final killSwitch = await killSwitchEnabled;
      final error = await charonErrorState;

      return {
        'state': state.index,
        'killSwitch': killSwitch,
        if (error != null) 'error': error.index,
      };
    } catch (e) {
      debugPrint('Error getting VPN status: $e');
      return {
        'state': FlutterVpnState.error.index,
        'killSwitch': false,
        'error': 'Failed to get VPN status: $e',
      };
    }
  }
}
