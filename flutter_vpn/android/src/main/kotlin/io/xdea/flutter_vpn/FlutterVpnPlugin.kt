...
class FlutterVpnPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    /**
     * Helper method to add app selection to VPN profile
     *
     * @param profileInfo The Bundle to add app selection to
     * @param mode The app handling mode:
     *   0 = All apps use VPN (SELECTED_APPS_DISABLE)
     *   1 = Selected apps are excluded from VPN (SELECTED_APPS_EXCLUDE)
     *   2 = Only selected apps use VPN (SELECTED_APPS_ONLY)
     * @param packageNames List of app package names to include/exclude
     */
    private fun addAppSelection(profileInfo: Bundle, mode: Int, packageNames: List<String>?) {
        profileInfo.putInt("AppHandling", mode)
        
        if (!packageNames.isNullOrEmpty()) {
            profileInfo.putStringArray("SelectedApps", packageNames.toTypedArray())
        }
    }
        ...
        when (call.method) {
            ...
            "connect" -> {
                ...
                if (args.containsKey("killSwitch")) {
                    profileInfo.putString("killSwitch", args["killSwitch"] as String)
                }

                if (args.containsKey("dnsServers")) {
                    profileInfo.putString("DnsServers", args["dnsServers"] as String)
                }

                // App selection handling
                if (args.containsKey("AppHandling")) {
                    val appHandling = (args["AppHandling"] as Int)
                    
                    @Suppress("UNCHECKED_CAST")
                    val selectedApps = if (args.containsKey("SelectedApps")) {
                        args["SelectedApps"] as List<String>
                    } else {
                        null
                    }
                    
                    addAppSelection(profileInfo, appHandling, selectedApps)
                }
