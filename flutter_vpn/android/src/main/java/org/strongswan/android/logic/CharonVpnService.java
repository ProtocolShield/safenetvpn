        ...
        if (intent != null) {
            ...
            if (VPN_SERVICE_ACTION.equals(intent.getAction()) || !DISCONNECT_ACTION.equals(intent.getAction())) {
                ...
                if (bundle != null) {
                    ...
                    // After creating the profile and setting its properties
                    Log.d(TAG, "Kill switch is setting");
                    if (bundle.containsKey("killSwitch")) {
                        if (bundle.getString("killSwitch").equals("true")) {
                            // let's log here to know if the kill switch is set
                            Log.d(TAG, "Kill switch is set to true");
                            profile.setSplitTunneling(VpnProfile.SPLIT_TUNNELING_BLOCK_IPV4 | VpnProfile.SPLIT_TUNNELING_BLOCK_IPV6);
                        } 
                    }

                    if (bundle.containsKey("DnsServers")) {
                        // let's log here to know if the DNS servers are set
                        Log.d(TAG, "DNS servers are set to: " + bundle.getString("DnsServers"));
                        profile.setDnsServers(bundle.getString("DnsServers"));
                        // profile.setDnsServers("2001:4860:4860::8888");
                    }
                    

                    // App selection handling
                if (bundle.containsKey("AppHandling")) {
                    int handling = bundle.getInt("AppHandling", 0);
                    // The handling values correspond to the enum ordinals
                    if (handling >= 0 && handling < VpnProfile.SelectedAppsHandling.values().length) {
                        profile.setSelectedAppsHandling(VpnProfile.SelectedAppsHandling.values()[handling]);
                        
                        // Get selected apps
                        if (bundle.containsKey("SelectedApps")) {
                            String[] packageNames = bundle.getStringArray("SelectedApps");
                            if (packageNames != null && packageNames.length > 0) {
                                // Add each package to the selected apps
                                SortedSet<String> currentApps = profile.getSelectedAppsSet();
                                currentApps.addAll(Arrays.asList(packageNames));
                                profile.setSelectedApps(currentApps);
                            }
                        }
                    }
                } else {
                    // Default behavior - all apps use the VPN
